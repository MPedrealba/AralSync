...
/** Recent assessments per student (date desc, capped at 8) for the full profile view. */
async function assessmentHistory(): Promise<Map<string, any[]>> {
  const assessments = (await Assessment.find().sort({ date: -1 }).lean()) as any[];
  const byStudent = new Map<string, any[]>();
  for (const a of assessments) {
    const sid = String(a.studentId);
    if (!byStudent.has(sid)) byStudent.set(sid, []);
    if (byStudent.get(sid)!.length < 8) {
      byStudent.get(sid)!.push({
        type: a.type,
        subject: a.subject || (a.type === 'READING_FLUENCY' ? 'Reading' : a.type === 'COMPREHENSION' ? 'Science' : '—'),
        title: a.title || a.type,
        score: a.score ?? null,
        masteryLevel: a.masteryLevel || null,
        combinedLevel: a.combinedLevel || null,
        wpm: a.wpm ?? null,
...
export async function GET(req: NextRequest) {
  try {
    ...
    const history = await assessmentHistory();
    ...
    const rows = records
      .map((r: any) => {
        ...
        return {
          ...
          section: r.section || '—',
          reading: readingLevels.get(sid) || 'Not Assessed',
          mastery: r.masteryStatus || null,
          guardian: r.guardian || '—',
          contact: r.contact || '—',
          address: r.address || '—',
    ...
    console.error('Coordinator learner-records API Error:', error);
