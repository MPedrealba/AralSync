...
export default function ReportsPage() {
  ...
  const [meta, setMeta] = useState<{ total: number; average?: number }>({ total: 0 });
  ...
  const generate = async (type?: string) => {
    ...
    setGenerated(false);
    try {
      const res = await fetch(`/api/teacher/reports?type=${t}`);
      const json = await res.json();
      if (json.success) {
        setRows(json.data.rows);
        setMeta(json.data.stats || { total: 0 });
        setGenerated(true);
      } else {
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {reportTypes.map((rt) => (
            ...
            >
              ...
              <div>
                <p className={`text-sm font-semibold ${selectedType === rt.id ? "text-blue-700" : "text-gray-800"}`}>
                  {rt.name}
                </p>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-gray-500">Report Type</label>
                <select
                  value={selectedType}
                ...
                <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-gray-400">
                      Generated {new Date().toLocaleString("en-US")} · {meta.total} rows
                      {meta.average ? ` · Avg ${meta.average}%` : ""}
                    </p>
                  </div>
                </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-100 bg-gray-50/60">
                        ...
                        <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Result</th>
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {rows.map((row, i) => (
                        <tr key={i} className="transition-colors hover:bg-gray-50/60">
                          <td className="px-6 py-3.5 text-sm font-medium text-gray-800">{row.name}</td>
                          <td className="px-6 py-3.5 text-sm text-gray-500">{row.lrn}</td>
                          <td className="px-6 py-3.5 text-sm text-gray-600">{row.grade}</td>
                          <td className="px-6 py-3.5 text-sm font-semibold text-gray-800">{row.score}</td>
                          <td className="px-6 py-3.5">
                            <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${row.ms}`}>{row.mastery}</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
